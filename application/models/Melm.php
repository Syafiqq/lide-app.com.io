 ...
 * Name         : syafiq
 * Date / Time  : 02 December 2016, 7:50 AM.
...
class Melm extends CI_Model
{
    ...
    public function assignOutputWeight($beta_topi)
    {
        $this->data['elm']['metadata']['beta_topi'] = new Matrix($beta_topi);
        $this->allowTest = true;
    }
    ...
    private function calculateELMTest($type)
    {
        $this->generateDataMatrix($type);
        $this->generateHInit($type);
        $this->generateH($type);
    ...
    public function getBetaTopi()
    {
        return $this->data['elm']['metadata']['beta_topi'];
    }
}
