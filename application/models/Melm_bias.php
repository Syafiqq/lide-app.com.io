...
class Melm_bias extends CI_Model
{
     ...
     * @param array $bias
     */
    public function add($profileID, $bias)
    {
        $this->delete($profileID);
        $query = 'INSERT INTO `elm_bias`(`profile`, `row`, `column`, `value`) VALUES ';
        $rm = count($bias) - 1;
        foreach ($bias as $rk => $rv)
        {
            $cm = count($rv) - 1;
            foreach ($rv as $ck => $cv)
            {
    ...
    {
        $query = 'DELETE FROM `elm_bias` WHERE `profile` = ?';
        $this->db->query($query, $profileID);
    }

    public function load($profileID)
    {
        $query = 'SELECT `column`, `value` FROM `elm_bias` WHERE `profile` = ? ORDER BY `row`, `column` ASC';
