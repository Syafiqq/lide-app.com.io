 ...
 * Name         : syafiq
 * Date / Time  : 11 November 2016, 11:10 AM.
...
<body>
<h1>HelloWorld</h1>
<form id="add" action="<?php echo site_url('ws/content/add') ?>" method="post">
    <input type="text" name="add" value="<?php echo (int)mt_rand(1, 100) ?>" required title="Add">
        ...
        $(function ()
        {
            ...
            $("#add").on('submit', function (event)
            {
                ...
                $.ajax({
                    ...
                    {
                        console.log(data);
                        $(form).children('input[name="add"]').val(Math.floor((Math.random() * 100) + 1));
                    })
                    .fail(function ()
                    {
                        alert("error");
